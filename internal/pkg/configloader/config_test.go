...
test:
  - name: github-repo-scaffolding-golang
	...
	Describe("Config method", func() {
		Context("ValidateDependency method", func() {
			It("should return array witch is not empty is tools dep error", func() {
				tools := []Tool{
					Tool{
						Name:       "test_tool",
						InstanceID: "0",
						DependsOn:  []string{"not_exist_dep_tool"},
					},
				}
				config := Config{
					Tools: tools,
				...
				Expect(errors[0].Error()).Should(ContainSubstring("dependency not_exist_dep_tool doesn't exist in the config"))
			})
			It("should return empty error array if dependency valid", func() {
				tools := []Tool{
					Tool{
						Name:       "exist_dep_tool",
						InstanceID: "1",
					...
					Tool{
						Name:       "test_tool",
						InstanceID: "0",
						DependsOn:  []string{"exist_dep_tool.1"},
					},
				}
				config := Config{
					Tools: tools,
