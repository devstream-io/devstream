	...
	for _, batch := range batchesOfTools {
		for _, tool := range batch {
			...
			if state == nil {
				...
				description := fmt.Sprintf("Tool %s (%s) found in config but doesn't exist in the state, will be created.", tool.InstanceID, tool.Name)
				...
				if drifted(tool.Options, state.Options) {
					...
					description := fmt.Sprintf("Tool %s (%s) config drifted from the state, will be updated.", tool.InstanceID, tool.Name)
					...
					if resource == nil {
						...
						description := fmt.Sprintf("Tool %s (%s) state found but it seems the tool isn't created, will be created.", tool.InstanceID, tool.Name)
						changes = append(changes, generateCreateAction(&tool, description))
					} else if drifted(resource, state.Resource) {
						// resource drifted from state, Update
						description := fmt.Sprintf("Tool %s (%s) drifted from the state, will be updated.", tool.InstanceID, tool.Name)
						changes = append(changes, generateUpdateAction(&tool, description))
					} else {
						// resource is the same as the state, do nothing
						log.Debugf("Tool %s (%s) is the same as the state, do nothing.", tool.InstanceID, tool.Name)
					}
				}
			}
...
func changesForDelete(smgr statemanager.Manager, cfg *configloader.Config, isForceDelete bool) ([]*Change, error) {
	...
	for i := len(batchesOfTools) - 1; i >= 0; i-- {
		...
		for _, tool := range batch {
			...
			description := fmt.Sprintf("Tool %s (%s) will be deleted.", tool.InstanceID, tool.Name)
